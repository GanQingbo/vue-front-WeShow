...
<script>
  ...
  export default {
    ...
    created() {
      const json = cookie.get("showType")
      if (json!='') {
        this.searchObj.showType = json
        console.log(this.searchObj.showType);
      }
      this.closed()
    },
    methods: {
      closed() {
        showApi.getShowListByQuery(this.searchObj).then(response => {
          ...
          cookie.set('showType', '', {domain: 'localhost'})
        })
      },
      showInfo(id) { //演出具体信息
      ...
      onSearch() {
        if(this.search==''){
          ...
          showApi.getShowByKey(this.search).then(response=>{
