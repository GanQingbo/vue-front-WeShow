<template>
  <div class="register">
    ...
    <van-form class="login-form">
      <van-field v-model="username" name="输入用户名" label="用户名" maxLength="20"
                 :rules="[{ required: true, message: '请输入用户名' }]">
      </van-field>
      <van-field v-model="number" name="输入手机号" label="手机号" type="tel" maxLength="11"
    ...
    methods:{
      ...
      getCode(){ //获取验证码
        if (this.isEmpty(this.number)) { Toast({message: '手机号不能为空', className: 'message-tips'}); }
          ...
          smsApi.getCode(this.number).then(response=>{
      ...
      register(){ //注册
        if (this.isEmpty(this.number)) { Toast({message: '手机号不能为空', className: 'message-tips'}); }
          ...
          this.uservo.username=this.username
          this.uservo.password=this.password1
          this.uservo.mobile=this.number
          this.uservo.code=this.code
          authApi.register(this.uservo).then(response=>{ //注册
            if(response.data.code==200){ //成功跳到登录页
              Toast({message: '注册成功', className: 'message-tips'});
              this.$router.replace('/Login')
            }else if(response.data.code==414){
              Toast({message: '验证码无效', className: 'message-tips'});
            }else if(response.data.code==411){
              Toast({message: '用户名已被注册', className: 'message-tips'});
            }else if(response.data.code==412){
              Toast({message: '手机号已被注册', className: 'message-tips'});
            }else if(response.data.code==413){
              Toast({message: '注册服务调用失败', className: 'message-tips'});
            }
          })
        }
      },
